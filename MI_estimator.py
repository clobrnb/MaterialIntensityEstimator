...
usage notes:
1. Recommended to run this code cell by cell, not the entire file at once
2. The cells labeled EDA (exploratory data analysis) are not part of the MI estimation pipeline. Skip them unless their resulting figures and statistics are needed.
3. The code uses relative paths to files. Change the directory in the chdir() function to your own local working direcory.
4. Versioning is based on dates
...
run_eda = False  # True will run the EDA code cells
analysis_figures = False  # True will run the post-estimaion analysis cells
...
dims_names = dims_names[7:]  # remove unused dimensions to keep things clean
...
if run_eda:
    ...
    # some very optional non-parametric statistical tests of similarity
...
stop_count = 30  # minimal MI datapoints before stopping the incrementation
export_to_excel = True  # opportunity to change your mind and export the final results. There's another one at the bottom of this cell for exporting all data
...
if export_to_excel:
    ...
    # mi_estimation_writer = pd.ExcelWriter('tests\\effects_of_n\\MI_ranges_n' + str(stop_count) + '_' + today + ".xlsx", engine='openpyxl')
...
for current_material in db_material_scores.query('score >= 2').index:
    ...
    for current_combi in mi_estimation_stats[current_material].index:  # running index for the current combination in mi_estimation
        ...
        # TODO in future versions: consider if stop_count or if stop_count-x to not expand to the entire world if we're already close to stop_count
        if current_count < stop_count:
...
# whether to export the full data to Excel
...
# %% analysis: visualize growth of pools by increment iteration, histograms (figure 2)
